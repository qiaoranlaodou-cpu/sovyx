...
class TestPipelineStateMachine:
    ...
    async def test_stt_rejection_reason_emits_dropped_event(
        ...
        caplog.set_level(logging.WARNING, logger=_TTS_CANCEL_LOGGER)
        await pipeline.start()
...
# Phase 5.F.24 — bypass coordinator methods extracted to
# ``_bypass_coordinator_mixin``; the deaf-signal-family events
# (``voice.deaf.*`` / ``voice_apo_bypass_*`` / ``audio.apo.bypassed`` /
...
# ``voice.tts.speaker_drift_detected``) now emit from that module's logger.
_TTS_CANCEL_LOGGER = "sovyx.voice.pipeline._tts_cancel_chain_mixin"
...
class TestPipelineHeartbeat:
    ...
    async def test_heartbeat_emits_with_max_probability(
        ...
        caplog.set_level(logging.INFO, logger=_BYPASS_LOGGER)
        caplog.set_level(logging.INFO, logger=_TTS_CANCEL_LOGGER)
...
class TestDeafSignalCoordinator:
    ...
    async def test_exhausted_strategies_emits_ineffective(
        ...
        caplog.set_level(logging.ERROR, logger=_BYPASS_LOGGER)
        caplog.set_level(logging.ERROR, logger=_TTS_CANCEL_LOGGER)
        callback = AsyncMock(
            return_value=[
                _bypass_outcome(
