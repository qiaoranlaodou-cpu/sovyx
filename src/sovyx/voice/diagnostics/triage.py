"""Triage analyzer: structured verdict from a sovyx-voice-diag tarball.
...
ranked RCA result -- as a typed :class:`TriageResult` dataclass for
in-process callers (e.g. ``sovyx doctor voice --full-diag``) or as
operator-facing markdown (back-compat with the v0.30.13 analyst CLI).
...
Module-level CLI (analyst-side, back-compat with the legacy
``tools/voice_diag_triage.py`` path)::

    python -m sovyx.voice.diagnostics.triage <tarball_path> [--out report.md]
    python -m sovyx.voice.diagnostics.triage --extract-dir <directory>
...
History: this module was extracted from ``tools/voice_diag_triage.py``
in v0.30.14 (T1.3 of MISSION-voice-self-calibrating-system-2026-05-05)
so the calibration engine and the new ``sovyx doctor voice --full-diag``
CLI can consume the typed verdict directly without re-implementing the
analysis. The legacy ``tools/voice_diag_triage.py`` path now imports
``_cli_main`` from here as a thin back-compat wrapper.
"""
