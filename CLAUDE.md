...
55. **Mission closure that references V-* operator-validation gates MUST land the corresponding rows in `OPERATOR-VALIDATION-BACKLOG-2026.md` in the SAME tag.** Reference: Mission B B-P0-4 — Mission A.1+A.2 finalization reports + tag-cut report + live-observation log all referenced V-A1-1 / V-A1-2 / V-A1-3 / V-A1-FINAL / V-A2-FINAL, but at v0.49.36 tag-cut the canonical operator-validation backlog `OPERATOR-VALIDATION-BACKLOG-2026.md` contained ZERO matches for `V-A1|V-A2|MISSION A`. Operator opened the backlog post-publish, found nothing to validate, and Mission B (BLOCKED on V-A1-FINAL live close per `feedback_validation_batching`) could not unblock through the canonical workflow. **Pattern:** any mission closure commit MUST land the V-* rows in the SAME commit OR a paired immediately-following commit. Backlog rows specify mechanical-vs-live closure criteria + falsifiability shape + sunset target where applicable. The mission finalization report + the backlog rows are SIBLING surfaces — the report carries the narrative, the backlog carries the operator-actionable checklist. **Detection:** pre-tag-cut `grep "V-<mission>-" OPERATOR-VALIDATION-BACKLOG-2026.md | wc -l == 0` for a mission whose finalization report references V-* IDs is a tag-cut hold. **Closure of Mission B B-P0-4.** **Sibling of #42 (composite store as the SINGLE operator-actionable surface — the backlog file is the SINGLE operator-validation surface).** Mission: `MISSION-B-REMEDIATION-PLAN-2026-05-21.md` §11.
