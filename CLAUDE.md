...
./scripts/verify_gates.sh     # runs all 10 gates + writes .git/.last-gates-pass marker
...
- **Tests:** 8, 9, 10, 12, 31
- **Architecture & Design:** 13, 16, 18, 19, 32, 33, 34, 37, 39, 40, 41, 42
...
42. **Operator-actionable degraded state MUST be surfaced through a single composite store/endpoint, never as N independent log emissions the operator is expected to correlate.** When a subsystem detects a degraded state that the operator can ACT on (failed initialization of a required dependency, missing required configuration, downstream dependency unavailable, performance threshold breached, recovery ladder exhausted), the detection site MUST: (a) emit the existing structured log line (operator playbooks reference these; renames are breaking changes); AND (b) call `EngineDegradedStore.record(DegradedEntry(...))` so the dashboard's composite banner + CLI doctor surface + `/api/engine/degraded` endpoint all see the state without log-grep. Three independent WARN lines that the operator has to correlate via log inspection ARE the anti-shape — they produce alert fatigue, hide the multi-axis correlation that the operator needs to diagnose, and leave the operator with no acknowledgement flow. Reference: Mission C4 — the v0.43.1 operator session emitted three structured WARNs (`no_llm_provider_detected` at `bootstrap.py:735`, `voice.factory.stt_language_unsupported` at `voice/factory/_validate.py:542`, `voice.failover.ladder_complete{verdict=exhausted}` at `voice/health/_runtime_failover.py`) over a 12-minute decorative-daemon window with ZERO dashboard surface and ZERO recovery action; the operator's only recourse was manual Ctrl-C. **Safe patterns:** (a) **`EngineDegradedStore.record` at every degraded-emit site** with axis + reason + severity + i18n token + action chips + metadata (Phase 1.A pattern); (b) **composite endpoint** at `/api/engine/degraded` returning the snapshot for dashboard banner + CLI doctor consumption; (c) **severity escalation by axis count** (1=warn, 2=error, 3+=critical) so the composite banner's visual weight reflects multi-axis correlation; (d) **server-side ack persistence** via the `operator_acks` SQLite table — never client-side `sessionStorage`/`localStorage` (multi-tab divergence, anti-pattern #19); (e) **TTL re-surface** so silenced acks reappear when the underlying condition persists (`_ack_resurface_scheduler`); (f) **auto-recovery governor** with bounded retry budget for the subset of degraded conditions that have a programmatic remediation path (Mission C4 Phase 2 soft-recovery governor for the voice pipeline). Quality Gate 10 (`scripts/dev/check_degraded_signal_surface.py`) AST-rejects any new `logger.warning(...)` emit matching `*_degraded` / `no_*_provider` / `*language_coerced` / `*language_unsupported` patterns that lacks a paired `EngineDegradedStore.record_*` (or `clear_axis`) call in the enclosing function. Pure platform-feature-availability gates (`*_unavailable` for module-init failures) are NOT enforced; they are developer-informational, not operator-actionable. Allowlist a false positive with an inline `# c4-allowlist: <rationale>` comment on the logger line. **Generalizes:** any subsystem with operator-actionable degraded state — bridge channels, plugin sandbox, persistence pool saturation, brain embedding model unavailable — falls under the same rule. Mission anchor: `docs-internal/missions/MISSION-c4-degraded-mode-banner-2026-05-17.md`. **Sibling of #40** (typed boundary drift): #40 governs the producer-to-boundary type contract; #42 governs the producer-to-operator surface contract. **Sibling of #41** (ladder iteration): #41 governs how a remediation candidate set is iterated once chosen; #42 governs how the operator is told that the remediation was needed at all.
